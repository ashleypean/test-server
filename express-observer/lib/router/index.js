...
proto.handle = function handle(req, res, out) {
  ...
  let errorSent = false;
  function next(err) {
    if(!observer.responses) {
      observer.responses = []
    }
    observer.responses.push({
      msg: 'router function',
      time: Date.now()
    })
    ...
    //pushing in route specific layer stack (including error handler)
        ...
        if(err) {
          ...
          //  stack: layer.handle.stack ? layer.handle.stack : []
          })
        } else {
            observer.stackLayers.push({
              executed: true,
    ...
    if(req.params.length) {
      observer.errorHandler.route = req.params
    }
    observer.errorHandler.error = err
